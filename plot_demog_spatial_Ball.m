...
function [Gbar,Ybins,Y2bins,avgB,avgP,figs] = plot_demog_spatial_Ball(B, P, prodB,tempChange,numYrs)
%Edward Tekwa May 1, 2018
...
%numYrs=20;
avgWindow=size(B,3)-numYrs:size(B,3); %average over last numYrs years
...
elseif numpatch==2
    ...
    plots=[3 ceil(numpatch/2) numpatch-2]; %[5 7 9]; 
end
...
numBins=14; %number of bins for heterotrophs (total number of bins is 1+numBins) 7
...
for pl=1:length(plots)
    ...
    xlabel('Time (yrs)','fontsize',16)
    ylabel('Biomass (log_{10} gm^{-3})','fontsize',16)
    set(gca,'fontsize',15);
    title(['Patch ' num2str(ptch) ', T=' num2str(P.T(ptch)) '+\Delta' num2str(tempChange)])
    box on;
    curYlim=ylim;
    ylim([log10(eps) 1]);
...
%    set(s,'MarkerEdgeColor',ProdErrColor);
    ...
    set(gca,'fontsize',15);
    box on;
    xlabel('log_{10}(body size [g])','fontsize',16)
    ylabel({['\fontsize{16} {\color{blue}biomass (log_{10} gm^{-3}) '],...
        ['\color{red}[productivity (log_{10} gm^{-3}/d)}']})
    %     ylabel(hAx(1),'Fish Biomass (log_{10} gm^{-3})','fontsize',16)
    %     ylabel(hAx(2),'Fish Productivity (log_{10} gm^{-3})','fontsize',16)
...
%     
    ...
    xlabel('log_{10}(body size [g])','fontsize',16)
    ylabel({['\fontsize{16} {\color{blue}biomass (log_{10} gm^{-3}) '],...
        ['\color{red}productivity (log_{10} gm^{-3}/day)}']});
    title(['patch ' num2str(ptch)])
    ax=gca;
...
% %ylim(plot2(1),[0 5])
% SpeciesDistr=area(([(mean(Z(:,:,avgWindow),3)),(mean(B(:,:,avgWindow),3))]),'LineStyle','none');
% SpeciesDistr(1).FaceColor='k';
% for i = 1:P.n
%     Ci = fix((log10(P.s.mi(i))-C(1))/(C(2)-C(1))*(size(CM,1)-1))+1;
%     SpeciesDistr(i+1).FaceColor=CM(Ci,:);
...
% title('Biomass distribution of all organisms')
% at = linspace(floor(log10(min(P.s.mi))), ceil(log10(max(P.s.mi))), 8);
% b = colorbar('YTick',[0:1/6:1],'YTickLabel',at);
% set(get(b,'ylabel'), 'String', 'Body Mass (log_{10} gm^{-3})', 'fontsize', 16)
% hold on
% yyaxis right
...
SpeciesDistr=area((mean(B(:,:,avgWindow),3)),'LineStyle','none');
hold on
for i = 1:P.n
    Ci = fix((log10(P.s.mi(i))-C(1))/(C(2)-C(1))*(size(CM,1)-1))+1;
    SpeciesDistr(i).FaceColor=CM(Ci,:);
    %plot(Xp, Y(:,i), 'Color', CM(Ci,:),'linewidth',2);
end
set(gca,'fontsize',15);
box on;
...
title('Biomass distribution (last 20 yrs)')
at = linspace(floor(log10(min(P.s.mi))), ceil(log10(max(P.s.mi))), 7);
b = colorbar('YTick',[0:1/6:1],'YTickLabel',at);
set(get(b,'ylabel'), 'String', 'log_{10}(body size [g])', 'fontsize', 16)
curYlim=ylim;
%ylim([0 2.5]);
xlim([0.8 numpatch+0.2]);
xticklabels(P.T+tempChange);
...
%[Ybins,Y2bins] = get_BPbins(B, P, prodB,numYrs); %test function
...
ylabel({['\fontsize{16} {\color{blue}biomass (log_{10} gm^{-3}) '],...
    ['\color{red}productivity (log_{10} gm^{-3}/day)}']});
title('global means (last 20 yrs)','fontsize',16)
ax=gca;
ax.XTick=[1:2+numBins]-0.5;
ax.XTickLabel=xbinLabels;
ax.YTick=[0:2:18];
...
% xlabel('Body Mass (log_{10} gm^{-3})','fontsize',16)
% ylabel('Productivity (log_{10} gm^{-3}/day)')
% ax=gca;
% ax.XTick=[1:8]-0.5;
% ax.XTickLabel=xbinLabels;
% ax.YTick=[0:2:18];
