...
P.Tend      = 200000 ./ P.dt; % number of days to run model forward in time 200000
%P.en        = 2; % number of ensemble runs
P.nx        = TR{II}.nx; % number of spatial patches (MUST be an even number)
P.dT        = 4./100000; % rate of temperature increase (deg C per day)
P.T         = [linspace(0,20,P.nx)  ]'; %temp gradient
...
P.lambda    = 0.4 ; % consumption efficiency (gi gj-1) originally: 0.4
P.L         = (P.s.mi ./ .025).^ (1./3.3) ./ 100; % length of individual in m;
P.Spd       = (1.*P.s.mi.^0.13)./100.*60.*60.*24; % swim speed from Megrey (mday-1)
P.ga        = 0.60; % fraction of day spent hunting
P.V         = P.ga.*pi.*(P.L.^2).*P.Spd./P.s.mi; % max search rate (m3 day-1 g-1)


%%---- movement parameters
% dx in m (explicit FTCS). 
% P.nx grid cells from -90 to 90 degN (180 deg) and 110,000 m per deg N.
P.Dmat      = diag(ones(1,P.nx-1),1) + diag(ones(1,P.nx-1),-1);
%P.Dmat(1,end) = 1; P.Dmat(end,1) = 1; % periodic boundaries
%P.Dmat      = zeros(P.nx, P.nx); % to turn off diffusion
P.alpha     = P.diff' .* (1./((110000*180/P.nx).^2));
