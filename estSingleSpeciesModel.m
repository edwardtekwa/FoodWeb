function [r,a,c,z,K,flag,raR2,r_T,K_T,K_T_ratio,r_T_ratio]=estSingleSpeciesModel(Btrans,dBtrans,gainBtrans,P,fitCode) %r, a dimensions: species, patch (temperature)
...
for species=1:size(Btrans,2)
    ...
    if ~isempty(Y4)
        ...
        [lm4,fval4,exitflag,output] = fminsearchbnd(@(params) LV_SS(Bseries,dBseries,Pseries,P.T,zs,Eas,ks,smis,Spds,fitCode,params),freeparstart,freeparmin,freeparmax,options); %estimate single-species growth model with skewed thermal envelope for intrinsic growth and temp-independent self-competition
        r(:,species)=lm4(1); %estimated r
        a(:,species)=lm4(2); %estimated a
        c(:,species)=lm4(3); %estimated c
