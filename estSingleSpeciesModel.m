...
for species=1:size(Btrans,2)
    ...
    if ~isempty(Y4)
        ...
        freeparmin=[0 -Inf 12];
        freeparmax=[Inf 0 12];
        [lm4,fval4,exitflag,output] = fminsearchbnd(@(params) LV_SS(Bseries,dBseries,Pseries,P.T,zs,fitCode,params),freeparstart,freeparmin,freeparmax); %estimate single-species growth model with skewed thermal envelope for intrinsic growth and temp-independent self-competition
        r(:,species)=lm4(1);
        a(:,species)=lm4(2);
        z(:,species)=lm4(3);
