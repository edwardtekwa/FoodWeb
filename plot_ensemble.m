...
function [] = plot_ensemble(Gbar,Ybins,Y2bins,avgB,avgP)
% ptch: patch to plot for time-series
scrsz = get(0,'ScreenSize');
fig=figure ('Color', [1 1 1],'Position',[1 scrsz(2) scrsz(3) scrsz(4)]);

avgWindow=ceil(length(B)*3/4):length(B)-1; %define time window to average long-term statistics
...
elseif numpatch==2
    ...
    plots=[1 ceil(numpatch/2) numpatch];
end
...
for pl=1:length(plots)
    ...
    hold on
    b=bar(nanmean(EnsembleGprodBins-floor(log10(eps)),'FaceColor',ProdColor);
...
ylabel('Biomass (g)','fontsize',16)
title('Biomass distribution of heterotrophs')
at = linspace(floor(log10(min(P.s.mi))), ceil(log10(max(P.s.mi))), 8);
b = colorbar('YTick',[0:1/7:1],'YTickLabel',at);
set(get(b,'ylabel'), 'String', 'Body Mass (log_{10} g)', 'fontsize', 16)
curYlim=ylim;
...
%heterotroph biomass, productivity, gamma richness, beta richness, gamma Shannon's div, beta
%Shannon's div, most common species (body mass)
